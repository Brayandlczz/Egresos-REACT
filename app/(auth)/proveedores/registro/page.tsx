...
const RegistroProveedor: React.FC = () => {
  ...
  useEffect(() => {
    const cargarPlanteles = async () => {
      ...
      else console.error('Error cargando planteles:', error)
    }
  ...
  const archivosRequeridos =
    formData.tipoPersona === 'Física' ? archivosFisica :
    formData.tipoPersona === 'Moral' ? archivosMoral : []
...
const uploads = await Promise.all(
  archivos.map(async (archivo, i) => {
    ...
    const consecutivo = String(i + 1).padStart(2, '0') // 01, 02, etc.
    ...
    const archivosFiltrados = uploads.filter((a) => a !== null)

...
const RegistroEtiqueta: React.FC = () => {
  ...
  const handleGuardar = async () => {
    ...
    const { error } = await supabase
      .from('etiquetas')
      .insert([{ nombre_etiqueta: nombreEtiqueta.trim(), plantel_id: plantelId }]);

    setTimeout(() => setLoading(false), 1000);

    if (error) {
      console.error('Error guardando la etiqueta:', error);
      alert('Error guardando la etiqueta: ' + JSON.stringify(error));
    } else {
      setSuccessMessage('¡Etiqueta registrada con éxito!');
      setPlantelId('');
      setNombreEtiqueta('');
      setTimeout(() => {
  ...
  return (
    <div className="relative p-8 bg-white-100 max-h-screen">
      ...
      <h2 className="text-2xl font-semibold mb-6">
        <span className="font-bold text-black">Etiquetas</span> | Registro de etiqueta de egreso
      </h2>
...
export default RegistroEtiqueta;

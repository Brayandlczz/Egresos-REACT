...
export default function PSTPMForm({
  onSubmit,
}: {
  onSubmit?: (values: PSTPMFormValues) => void;
}) {
  ...
  const restantePct = form.pagoEsquema === 'anticipo_2' ? (100 - anticipoPctNum - segundoPagoPctNum) : (100 - anticipoPctNum);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    const coloresArray =
      form.coloresMultiline
        .split('\n')
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    const payload: any = {
      ...
      colores: coloresArray.length ? coloresArray : undefined,
    ...
    onSubmit?.(payload as unknown as PSTPMFormValues);
  };

  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-5xl p-6 space-y-6">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-slate-800">Contrato de Servicio · Persona Moral</h1>
      ...
      <section className={section}>
        <h2 className={legend}>B) Proveedor (Persona Moral)</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              onChange={onChange}
              className={baseInput}
              placeholder="p. ej. Juan Manuel Estrada Solís"
              required
            />
          </div>
      ...
      <section className={section}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2">
            <label className={baseLabel}>Detalle por colores/alcances (uno por línea)</label>
            <textarea
              name="coloresMultiline"
              value={form.coloresMultiline}
              onChange={onChange}
              rows={4}
              className={baseInput}
              placeholder={`1.- Azul. Las partes internas.\n2.- Verde. Las columnas.\n3.- Amarillo. Ventanas y puertas.\n4.- Rojo. Herrería/Metales.`}
            />
            <p className="text-xs text-slate-500 mt-1">Se enviará como <code>string[]</code> en <strong>values.colores</strong>.</p>
          </div>
        </div>
      </section>

      <section className={section}>
        <h2 className={legend}>D) Datos notariales (Persona Moral)</h2>
      ...
      <section className={section}>
        <h2 className={legend}>F) Plazos y garantías</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>

...
type Props = {
  values: PSTPMFormValues;
  autoExportOnValuesChange?: boolean;
  onExportDone?: () => void;
};

export default function ContratoPSTPM({
  values,
  ...
  const generarDocx = async () => {
    ...
    const pagoMedioMap: Record<PSTPMFormValues["pagoMedio"], string> = {
      transferencia: "transferencia electrónica a la cuenta bancaria del prestador de servicio",
    ...
    const pagoParrafos = (() => {
      ...
      const base: Paragraph[] = [
        ...
        new Paragraph({
          ...
          children: [
            new TextRun(
              `B). - ${values.fechaSegundoPago || "a la entrega"} se pagará el ${segundoPct}% pendiente de pago, el cual corresponde a ${mxn(segundoMonto)} (${numeroALetrasMX(segundoMonto)}) IVA incluido.`
            ),
          ],
        }),
      ];
    ...
    const parrafosObjetoDetallado: Paragraph[] = [];
    if (Array.isArray((values as any).colores) && (values as any).colores.length) {
      (values as any).colores.forEach((linea: string) => {
        if (!linea) return;
        parrafosObjetoDetallado.push(
          new Paragraph({ spacing: { after: 100 }, children: [new TextRun(linea)] })
        );
      });
      parrafosObjetoDetallado.push(
        new Paragraph({
          spacing: { after: 200 },
          children: [new TextRun("DESCRIBIR DETALLADAMENTE LOS COLORES DE LAS PAREDES, HERRERÍAS O METALES, BAÑOS, ETC.  (Todos los servicios deberán ser descritos).")],
        })
      );
    } else if (values.objetoLargo) {
      parrafosObjetoDetallado.push(
        new Paragraph({
          spacing: { after: 200 },
          children: [new TextRun(values.objetoLargo)],
        })
      );
    }

    const razonSocial = (values as any).proveedorRazonSocial || values.proveedorNombre || "_________________________";
    const repLegal = (values as any).representanteLegalNombre || values.proveedorNombre || "_________________________";
    const escrituraNumero = (values as any).escrituraNumero || "__________";
    const volumenNumero = (values as any).volumenNumero || "__________";
    const notarioNombre = (values as any).notarioNombre || "_________________________";
    const notariaNumero = (values as any).notariaNumero || "___";
    const estadoNotaria = (values as any).estadoNotaria || "__________";
      ...
      sections: [
        {
          ...
          children: [
            new Paragraph({
              ...
              children: [new TextRun({ text: "CONTRATO PST PM.", bold: true, size: 24 })],
            }),
            ...
            new Paragraph({
              ...
              children: [
                new TextRun(
                  `En la ciudad de Tuxtla Gutiérrez, Chiapas; a ${fecha}, comparecen por una parte la Universidad Internacional del Conocimiento e Investigación, S. C. a través de su representante legal Dra. María Xóchilt Ortega Grillasca, quien en lo sucesivo se denominará la “contratante”, y por la otra la empresa denominada ${razonSocial} (proveedor), representada por su apoderado legal el C. ${repLegal}, quien en lo sucesivo se denominará el “prestador de servicio”; ambas partes manifiestan tener concertado un contrato de prestación de ${values.tipoPrestacion || "servicio técnico"} ${values.objetoCorto ? `de ${values.objetoCorto}` : ""} que formalizan al tenor de las siguientes declaraciones y cláusulas:`
                ),
              ],
            }),
            ...
            new Paragraph({
              spacing: { after: 400 },
              children: [new TextRun(`6.- Que le es indispensable para cumplir con su objeto social, contratar el servicio ${values.objetoLargo || "(describir objeto completo)"} `)],
            }),

            new Paragraph({ spacing: { after: 200 }, children: [new TextRun("B). - DEL PRESTADOR DE SERVICIO TÉCNICO.")] }),
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`1.- Manifiesta la empresa denominada ${razonSocial}, ser una persona moral constituida conforme a la legislación mexicana, lo cual acredita con el Instrumento Notarial número ${escrituraNumero}, Volumen Número ${volumenNumero}, expedida por ${notarioNombre}, Notaría Pública número ${notariaNumero} del Estado de ${estadoNotaria}.`)]
            }),
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`2.- Que en este acto es representada por el C. ${repLegal}, en su carácter de representante legal, el que acredita mediante el instrumento señalado en el numeral anterior a la fecha de la celebración del presente contrato.`)],
            }),
            new Paragraph({
              spacing: { after: 200 },
            ...
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`5.- Que su actividad preponderante es ${values.actividadPreponderante || "_________________________"} .`)],
            }),
            new Paragraph({
              spacing: { after: 400 },
            ...
            new Paragraph({
              spacing: { after: 400 },
              children: [new TextRun(`Con las declaraciones anteriores, las referidas partes han acordado celebrar un contrato de ${values.tipoPrestacion || "prestación de servicio técnico" }, el cual se sujeta a las siguientes:`)],
            }),
            ...
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`UNO. OBJETO DEL CONTRATO. - La contratante manifiesta que, debido a ${values.objetoCorto || "(describir causa u objeto corto)"}.`)]
            }),
            ...parrafosObjetoDetallado,
            ...
            new Paragraph({ spacing: { after: 200 }, children: [new TextRun("7.- Carátula de estado de cuenta donde se visualice la CLABE y número de cuenta (no mayor a dos meses).")] }),
            ...
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`CUATRO. EL PAGO. - El pago del servicio se hará a través de ${pagoMedioTxt} previo envío del CFDI. Por el sólo hecho de hacerse la transferencia de pago se tendrá por aceptado el pago por el prestador de servicio, sin más requisito alguno.`)]
            }),
            ...
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`SEIS. DE LA GARANTÍA. - El prestador de servicio ofrece como garantía de su servicio el plazo de ${values.plazoGarantiaDias || 30} días contado a partir de que entregue las instalaciones. Para hacer efectiva la garantía bastará con que la contratante dé aviso por teléfono al prestador de servicio.`)]
            }),

            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`SIETE. PLAZO DE CONCLUSIÓN. - Ambos acuerdan que el plazo para la entrega del servicio solicitado sea concluido en ${values.plazoConclusionDias || 15} días naturales contados a partir de la firma del presente contrato.`)]
            }),
            ...
            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`DIEZ. PENA CONVENCIONAL. - Las partes acuerdan que en el caso de que EL PRESTADOR DE SERVICIO no haga la entrega en el plazo de ${values.diasPlazoPena || values.plazoConclusionDias || 15} días naturales establecido en la cláusula SIETE, o LA CONTRATANTE no pague después de concluido los servicios, pagará por mora el nueve (9%) por ciento sobre el monto establecido como precio de la contraprestación.`)]
            }),

            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`ONCE. DAÑOS Y PERJUICIOS. - EL PRESTADOR DE SERVICIO se hará responsable de los daños y perjuicios que sufra LA CONTRATANTE en caso de que no dé cumplimiento al objeto del contrato, ya sea por negligencia, o por utilizar personal que no tenga los conocimientos técnicos (${values.objetoCorto || "describir servicio"}). Siendo a costa del PRESTADOR DE SERVICIO la mano de obra y el material de insumo que se requiera para que la Institución Educativa cumpla con el objetivo solicitado por LA CONTRATANTE.`)]
            }),
            new Paragraph({
              spacing: { after: 400 },
              children: [new TextRun("En caso de que EL PRESTADOR DE SERVICIO no dé cumplimiento con el objeto del contrato, se podrá optar que pague en efectivo los daños y perjuicios ocasionados, previa cuantificación por un tercero con experiencia en la materia.")]
            }),

            new Paragraph({
              spacing: { after: 200 },
              children: [new TextRun(`DOCE. - Ambas partes manifiestan que, en el caso de que exista controversia sobre la aplicación del presente contrato, se someten a la jurisdicción de las autoridades de la ciudad de ${values.municipioNombre || "Tapachula de Córdova y Ordoñez, Chiapas / Tuxtla Gutiérrez, Chiapas"}.`)]
            }),
    ...
    saveAs(blob, "Contrato_PST_PM.docx");
  };
